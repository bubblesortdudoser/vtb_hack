...
class Parser:
    ...
    service = Service('/home/donqhomo/Desktop/vtb_hack/server/chromedriver')
...
def interfax_all_news_business(parser)->'News from interfax':
    try:
        logMessage = parser.virtual_display()
        logging.info(logMessage)
        driver = webdriver.Chrome(service=parser.service, options=parser.options)
        driver.get(parser.interfax_business_url)
        time.sleep(1)
